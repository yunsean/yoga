...
public class HttpUtils {
	...
	public static String execute(String method, String url, String body, String contentType, boolean checkStatusCode) throws Exception {
		return execute(method, url, body, contentType, null, checkStatusCode);
	}
	public static String execute(String method, String url, String body, String contentType, Map<String, Object> headers, boolean checkStatusCode) throws Exception {
		try {
			URL uri = new URL(url);
			...
			if (headers != null) headers.forEach((k, v)-> urlConnection.setRequestProperty(k, v == null ? "" : v.toString()));
	...
	public static String getExecute(String url, Map<String, Object> params) throws Exception {
		String query = HttpUtils.keyValues(params);
	...
	public static String postExecute(String url, Map<String, Object> body) throws Exception {
		String bodyStr = HttpUtils.keyValues(body);
		...
		if (keyValues != null && keyValues.length > 1) {
			for (int i = 0; i < keyValues.length - 1; i += 2) {
				if (keyValues[i + 1] != null) {
					result += HttpUtils.addKeyValue(keyValues[i].toString(), keyValues[i + 1].toString());
				} else {
					result += HttpUtils.addKeyValue(keyValues[i].toString(), "");
				}
			}
	...
	public static String keyValues(Map<String, Object> keyValues) {
		...
		for (String key : keys) {
			result += HttpUtils.addKeyValue(key, keyValues.get(key));
		}
		if (result.length() > 0) {
			result = result.substring(0, result.length() - 1);
		}
