        ...
        <div class="modal-body">
            <div class="modalForm">
                <div class="form-group">
                    ...
                    <label class="col-sm-2 control-label">默认注册用户所在<@common.deptAlias/>：</label>
                    <div class="col-sm-4">
                        <select class="form-control" style="width:100%" id="dept_select">
                        ...
                        <script>
                            $("#dept_select").val('${param.value?if_exists?c}');
                        </script>
                    </div>
                    <label class="col-sm-3"></label>
                </div>
...
<script>
    function save() {
        var value = $("#dept_select").val();
        var showValue = $("#dept_select").find("option:selected").text();
        $.post(
                "/api/dept/setting/save",
                {deptId: value, deptName: showValue},
                function (data) {
                    if (data.code < 0) {
