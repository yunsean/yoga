...
<div class="container-fluid">
    <div class="row">
        <ol class="breadcrumb">
            <li><a href="/tenant/tenants"><i class="fa fa-dashboard"></i>租户管理</a></li>
            <li>自定义菜单</li>
                    ...
                    <div class="tableToolContainer" style="margin-bottom:15px">
                        <form action="/tenant/tenants" class="form-inline">
                            <@shiro.hasPermission name="gbl_tenant.add" >
                                <a href="#" class="btn btn-primary" id="tenantAddButton">
...
<script>
        ...
        $.post(
                ...
                function (data) {
                    if (data.code < 0) {
                        ...
                        window.location.href = "/tenant/menus?tenantId=${tenantId?c}";
                    }
                },
                "json"
        );
    ...
    function doDelete(id) {
        warningModal("确定要删除该菜单项吗?", function () {
            $.get(
                    "/api/tenant/menu/delete?menuId=" + id,
                    function (data) {
                        if (data.code < 0) {
                            alertShow("danger", data.message, 3000);
                        } else {
...
<script>
    function addMenu() {
        ...
        $.post(
                "/api/tenant/menu/add",
                {tenantId: ${tenantId?c}, code: code, group: group, name: name, url: url, remark: remark, sort: sort},
                function (data) {
                    if (data.code < 0) {
                        alertShow("danger", data.message, 3000);
                    } else {
