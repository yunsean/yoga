    ...
    public List<MenuItem> getAllMenus(boolean excludeEmptyUrl) {
        ...
        for (MenuItem menu : menuItems) {
            ...
            for (MenuItem sub : menu.getChildren()) {
                if (!excludeEmptyUrl || StrUtil.isNotBlank(sub.getUrl())) {
    ...
    public List<MenuItem> getMenuByModule(Set<String> modules, boolean urlOnly, boolean noConfig) {
        ...
        for (MenuItem menu : menuItems) {
            ...
            for (MenuItem sub : menu.getChildren()) {
                ...
                if (urlOnly && StrUtil.isBlank(sub.getUrl())) continue;
                if (noConfig && StrUtil.isBlank(sub.getUrl()) && (sub.getChildren() == null || sub.getChildren().size() < 1)) continue;
