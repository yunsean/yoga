...
<@html>
    ...
    <@bodyFrame>
        <@crumbRoot name="${tenantAlias!''}管理" icon="icon-user">
            ...
            <@crumbItem href="#" name="${tenantAlias!''}菜单" />
            <@backButton/>
        </@crumbRoot>
        <@bodyContent>
            <@panel>
                <@panelHeading "${tenantAlias!''}菜单" />
                <@panelBody>
                    <@rightAction>
    ...
    <script>
        function doDelete(id) {
            warningModal("确定要删除该菜单项吗?", function () {
                $.ajax({
                    url: "/admin/tenant/tenant/menu/delete.json?tenantId=${(tenantId?c)!}&menuId=" + id,
                    type: 'DELETE',
...
<script>
    ...
    function doEdit(id) {
        $("#add_form")[0].reset();
        $.get(
                "/admin/tenant/tenant/menu/get.json?tenantId=${(tenantId?c)!}&menuId=" + id,
                function (data) {
    ...
    function doSave() {
        ...
        $.post(id == 0 ? "/admin/tenant/tenant/menu/add.json" : "/admin/tenant/tenant/menu/update.json",
                json,
                function (data) {
                    if (data.code < 0) {
