...
public class Money implements Serializable, Comparable {
    ...
    public String getYuan() {
        if (this.getAmount().intValue() >= 10000) {
            return this.getAmount().intValue() / 10000 + "万";
        } else {
    ...
    public String getFormatCurrency() {
        String currecy = this.currency.getSymbol() + this.getFormatNumber();
        return currecy;
    }
    ...
    public String getFormatNumber() {
        ...
        String currecy = format.format(this.getAmount());
        return currecy;
    }
    
    public static Money createByCent(long cent){
