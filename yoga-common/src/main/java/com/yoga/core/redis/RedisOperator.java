...
import com.yoga.core.utils.JsonOperator;
...
public class RedisOperator{
	...
	public <HK, HV> void hashSet(String key, Map<HK, HV> values){
		...
		for(Map.Entry<HK, HV> v : values.entrySet()){
			waitPut.put(JsonOperator.toJson(v.getKey()), JsonOperator.toJson(v.getValue()));
		}
	...
	public <HK, HV> Map<HK, HV> hashEntries(String key, Class<?> hkClass, Class<?> hvClass){
		...
		for(Map.Entry<String, String> v : saved.entrySet()){
			target.put(JsonOperator.toObject(v.getKey(), hkClass), JsonOperator.toObject(v.getValue(), hvClass));
		}
		return target;
	}
	...
	public <HK, HV> HV hashGet(String key, HK hashKey, Class<?> hvClass){
		...
		String saved = getOpsForHash().get(key, JsonOperator.toJson(hashKey));
		if(null == saved){
			return null;
		}
		return JsonOperator.toObject(saved, hvClass);
	}
	...
	public <HK, HV> List<HV> hashGets(String key, Collection<HK> hashKeys, Class<?> hvClass){
		...
		for(String v : saved){
			target.add(JsonOperator.toObject(v, hvClass));
		}
		return target;
	}
	...
	private static <HK> List<String> convertToHK(Collection<HK> hashKeys){
		...
		for(HK hashKey : hashKeys){
			hashKeyStrs.add(JsonOperator.toJson(hashKey));
		}
		return hashKeyStrs;
	}
